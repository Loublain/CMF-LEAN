...
We show that a CMF over the Gaussian integers
naturally admits two distinct integrability conditions on the unit
plaquette: \(S - T = 0\) (commutative, the original Raz--Kaminer
setting) and \(S + T = 0\) (anticommutative, naturally realised in the
Pauli algebra and algebraically equivalent to spinors). These two
conditions define separate families that cannot coexist nontrivially in
a single matrix field. We identify known constant pairs
...
\hypertarget{two-integrability-conditions}{%
\section{2. Two Integrability Conditions on
\(\mathbb{Z}\){[}i{]}}\label{two-integrability-conditions}}
...
\(\mathbb{Z}\){[}i{]} rather than \(\mathbb{Z}^{2}\), the
unit-plaquette closure admits two algebraically distinct integrability
conditions, depending on whether the two paths around the plaquette
have equal or opposite-sign matrix products.

\textbf{Definition 2.1} (Two integrability conditions). Setting

\[S(m,n) = M_y(m+1,n)\cdot M_x(m,n) \quad \text{and} \quad T(m,n) = M_x(m,n+1)\cdot M_y(m,n)\]

for the two products around a unit lattice plaquette, we call a CMF
\emph{commutative} if \(S - T = 0\) throughout the lattice, and
\emph{anticommutative} if \(S + T = 0\):

\[S - T = 0 \quad \text{(commutative CMF)}; \qquad S + T = 0 \quad \text{(anticommutative CMF)}\]

The anticommutative condition \{M\_x, M\_y\} = 0 admits a natural
realisation in the Pauli algebra: writing M\_x = a\(\sigma_{1}\) +
b\(\sigma_{3}\) and M\_y = c\(\sigma_{2}\), the Clifford anticommutation
\(\sigma _{i}\sigma _{j}\) + \(\sigma _{j}\sigma _{i}\) =
2\(\delta _{ij}\)I (for i \(\neq\) j) yields \{M\_x, M\_y\} = 0. The
commutator {[}M\_x, M\_y{]} = 2ic(a\(\sigma_{3}\) − b\(\sigma_{1}\)) is
generically nonzero, so this realisation does \emph{not} satisfy the
commutative condition --- consistent with the explicit
c\textsubscript{1} computation in §2.2 below, where S + T = 0 holds and
S \(\neq\) T. A field satisfying both conditions simultaneously would
require M\_xM\_y = M\_yM\_x = 0, which is incompatible with the nonzero
polynomial step-matrix structure of the CMF construction. We treat the
commutative (S − T = 0) and anticommutative (S + T = 0) branches as two
distinct CMF families on \(\mathbb{Z}\)\([i]\), with the Pauli
...
The degree-drop pattern (\(\pi^{2}\) paired with \(\pi\)) reflects the
matrix algebra of the two sectors: in the anticommutative (Pauli)
realisation, the step matrix M = r\(\sigma_{z}\) + p\(\sigma_{x}\)
satisfies M\textsuperscript{2} = (r\textsuperscript{2}+p\textsuperscript{2})\(\cdot\)I, so
matrix-level squaring produces a scalar in (r\textsuperscript{2}+p\textsuperscript{2})
--- a linear quantity in the polynomial entries. The commutative sector
lacks this involutive structure; the natural accumulator is the full
matrix product, which the Raz--Kaminer setting shows converges to
\(\pi^{2n}\) for the appropriate hypergeometric step matrices. This is
consistent with the observed pairing of even \(\zeta\)(2n) (closed
forms in powers of \(\pi\)) with odd \(\zeta\)(2n−1) (no such forms ---
they live on the anticommutative sector), though the heuristic does not
by itself constitute a proof.
